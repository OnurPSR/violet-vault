...
Use Markdown for linear material, LaTeX for compact mathematical notation, Mermaid for discrete edge logic, and Excalidraw for free placement, geometry, large annotated matrices, or spatial relationships.

Do not add a visual when prose or a short equation expresses the same meaning more clearly.
...
## Create assets before embedding

- Resolve the vault's established asset directories and naming pattern.
- Create the editable source first when the format has one.
- Export and verify the final render before inserting its embed.
- Keep Excalidraw source, SVG export, and embed names synchronized.
- Do not insert a placeholder, guessed path, or embed for an asset that was not created successfully.

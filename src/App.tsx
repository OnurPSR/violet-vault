...
import type { AgentId, AppState, Attachment, CliStatus, Conversation, Effort, Message, Note, ProviderId } from "./types";
...
const AGENTS = [
  ...
  { id: "supervisor" as AgentId, name: "Vault Supervisor", mode: "Audit + route", text: "Plan, route and independently verify the three specialist agents.", icon: ShieldCheck, color: "#e06cff" },
];
...
const SUGGESTIONS: Record<AgentId, string[]> = {
  ...
  supervisor: ["Audit this reconstruction before writing", "Route this task to the safest agent", "Review all proposed vault changes"],
};
...
export default function App() {
  ...
  function upsertConversation(next: Message[], title: string, id: string) {
    ...
    setChatId(id);
    setChats((all) => [item, ...all.filter((chat) => chat.id !== id)]);
  }

  async function fresh(nextAgent = agentId) {
    if (running) await window.violet.stopAgent();
    setRunning(false);
    setChatId(null);
    setMessages([]);
    setPrompt("");
  ...
  return (
    <main className="app-shell">
      <aside className={`history-rail ${menu ? "mobile-open" : ""}`}>
        <div className="brand-mark">V</div>
        <button className="rail-action primary" onClick={() => void fresh()} title="New conversation"><MessageSquarePlus size={19} /></button>
        <div className="rail-divider" />
        <div className="history-stack">
          {chats.slice(0, 7).map((chat) => (
            <button key={chat.id} className={`history-dot ${chatId === chat.id ? "active" : ""}`} onClick={() => openChat(chat)} title={chat.title}>
              {chat.title[0]?.toUpperCase()}
            </button>
          ))}
        </div>
        <div className="rail-spacer" />
        <button className="rail-action" title="Conversations are stored locally"><Archive size={18} /></button>
        <button className="rail-action" title="Configuration follows the selected controls"><Settings2 size={18} /></button>
      </aside>
      ...
      <aside className={`agent-sidebar ${menu ? "mobile-open" : ""}`}>
        ...
        <section className="sidebar-section">
          <div className="section-label"><span>AGENTS</span><MoreHorizontal size={15} /></div>
          <div className="agent-list">
            {AGENTS.map((item) => {
              const Icon = item.icon;
              return (
                <button key={item.id} className={`agent-item ${agentId === item.id ? "active" : ""}`} onClick={() => chatId ? void fresh(item.id) : setAgentId(item.id)}>
        ...
        <section className="sidebar-section recent-section">
          ...
          <div className="recent-list">
            ...
            {chats.slice(0, 12).map((chat) => <button key={chat.id} onClick={() => openChat(chat)}><span>{chat.title}</span><small>{age(chat.updatedAt)}</small></button>)}
          </div>
        </section>
        <div className="sidebar-footer">
          ...
          <button className="icon-button" onClick={() => window.violet.checkCli().then(setCliStatus)} title="Refresh CLI status"><Settings2 size={16} /></button>
        </div>
      </aside>
      ...
      <section className="chat-workspace">
        <header className="topbar">
          ...
          <div className="model-controls">
            <label className="select-control provider-select"><Bot size={15} /><select value={provider} disabled={running} onChange={(event) => chooseProvider(event.target.value as ProviderId)}>{Object.entries(PROVIDERS).map(([id, item]) => <option key={id} value={id}>{item.label}</option>)}</select><ChevronDown size={13} /></label>
            <label className="select-control model-select"><select value={model} disabled={running || provider === "local"} onChange={(event) => setModel(event.target.value)}>{PROVIDERS[provider].models.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select><ChevronDown size={13} /></label>
            <label className="select-control effort-select"><BrainCircuit size={14} /><select value={effort} disabled={running || provider === "local"} onChange={(event) => setEffort(event.target.value as Effort)}><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select><ChevronDown size={13} /></label>
          </div>
        </header>
        ...
        <div className="composer-region">
          ...
          <p className="composer-hint">{agentId === "retriever" ? "Retriever is enforced as read-only." : "Writes are restricted to the selected vault and governed by the active agent contract."}</p>
        </div>
      </section>

      <aside className="context-panel">
        <div className="context-heading"><div><span className="micro-label">CONTEXT</span><h2>Obsidian Vault</h2></div><button className="icon-button" title={vaultPath ?? "No vault selected"}><MoreHorizontal size={17} /></button></div>
        <button className="vault-picker" onClick={() => void selectVault()}><span className="vault-icon"><Library size={19} /></span><span><small>ACTIVE VAULT</small><strong>{vaultName}</strong></span><FolderOpen size={17} /></button>
        <div className="note-search"><Search size={15} /><input ref={searchInput} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search notes" /><span>Ctrl K</span></div>
        <div className="file-browser">
          {notes.length === 0 ? (
            ...
            <><div className="browser-summary"><span>{filtered.length}{filtered.length < notes.length ? ` of ${notes.length}` : ""} notes</span><span>Markdown</span></div><div className="note-list">{filtered.map((item) => <button key={item.path} className={note?.path === item.path ? "active" : ""} onClick={() => void openNote(item)}><FileText size={15} /><span><strong>{item.name}</strong><small>{item.path.split("/").slice(0, -1).join(" / ") || "Vault root"}</small></span></button>)}</div></>
          )}
        </div>
